...
#    shiftX = imgA.shape[1]
    ...
    for i in range(X1.shape[0]):
        ...
        cv2.circle(newImg, (X1[i], Y1[i]), 3, cur_color, 2)
        cv2.circle(newImg, (X1[i], Y1[i]), 3, [0, 0, 0], 2)
        cv2.circle(newImg, (X2[i], Y2[i]+shiftX), 3, cur_color, 2)
        cv2.circle(newImg, (X2[i], Y2[i]+shiftX), 3, [0, 0, 0], 2)
...
def show_correspondence2(imgA, imgB, X1, Y1, X2, Y2, file_name='result'):
    ...
    for i in range(X1.shape[0]):
        ...
        cv2.circle(newImg, (X1[i], Y1[i]), 3, cur_color, 2)
        cv2.circle(newImg, (X2[i], Y2[i]+shiftX), 3, cur_color, 2)
        cv2.line(newImg, (X1[i], Y1[i]), (X2[i], Y2[i]+shiftX), cur_color, 1)

    print('Saving visualization to vis_arrows_' + file_name + '.png')
    cv2.imwrite('vis_arrows_'+file_name+'.png', newImg*255.0)

def show_correspondence3(imgA, X1, Y1, X2, Y2, file_name='result'):
    ############ verify  ###########################
    Height = 2*imgA.shape[0] 
    Width = imgA.shape[1]
